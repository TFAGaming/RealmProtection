            ...
            if (!LandsManager.hasLand(player.getName())) {
                ...
                if (args.length == 2) {
                    Inventory inventory = Bukkit.createInventory(player, 9 * 3,
                            ColoredString.translate("&9Nature Flags"));
                    ...
                    for (List<Object> flag : allflags) {
                        ...
                        flagButtonMeta.setDisplayName(ColoredString.translate("&7" + flag.get(0)));

                        ArrayList<String> flagButtonLore = new ArrayList<>();

                        if ((Boolean) flag.get(1) == true) {
                            flagButtonLore.add(ColoredString.translate("&aEnabled"));
                        } else {
                            flagButtonLore.add(ColoredString.translate("&cDisabled"));
                        }
                    ...
                    ItemStack landNameButton = new ItemStack(Material.PAPER);
                    ItemMeta landNameButtonMeta = landNameButton.getItemMeta();
                    landNameButtonMeta.setDisplayName(ColoredString.translate("&6Land: &r" + land_name));
                    landNameButton.setItemMeta(landNameButtonMeta);

                    ItemStack closeButton = new ItemStack(Material.BARRIER);
                    ItemMeta closeButtonMeta = closeButton.getItemMeta();
                    closeButtonMeta.setDisplayName(ColoredString.translate("&cClose"));
