...
public class DepositCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 1) {
                player.sendMessage(ColoredString.translate(LoadConfig.commandString("deposit.no_amount_provided")));
                return true;
            }
            ...
            try {
                ...
                if (balance_input <= 0) {
                    player.sendMessage(ColoredString.translate(LoadConfig.commandString("deposit.input_negative_or_zero")));
                    return true;
                }

                if (balance_input > player_balance) {
                    player.sendMessage(ColoredString.translate(LoadConfig.commandString("deposit.input_larger_than_player_balance")));
                    return true;
                }
                ...
                player.sendMessage(ColoredString.translate(LoadConfig.commandString("deposit.deposited_success").replace("%amount%", args[1]).replace("%land_balance%", String.format("%.2f", (land_balance + balance_input)))));

            } catch (NumberFormatException err) {
                player.sendMessage(ColoredString.translate(LoadConfig.commandString("deposit.input_not_double_or_integer")));
            }
